...
void drawAzzan(int DrawAdd)
{
  ...
  if ((ct % 2) == 0)
  {
    ...
    if (ct <= 20)
    {
     digitalWrite(23, HIGH); //32
    }
  }
  else
  {
   digitalWrite(23, LOW); //32
  }
    ...
    if((Tmr-lsRn) > 1000 and ct <=cn_l)
      {
          ...
          scd = (cn_l-ct)%60;
          if(mnt>0) {
          Disp->setFont(&FreeMonoBoldOblique9pt7b);sprintf(locBuff,"%02d:%02d",mnt,scd);Disp->setTextColor(HIJAU);drawText(15, 22,locBuff);}
          else {
          Disp->setFont(&FreeMonoBoldOblique9pt7b);sprintf(locBuff,"%02d",scd);Disp->setTextColor(HIJAU);drawText(30, 22,locBuff);}    
